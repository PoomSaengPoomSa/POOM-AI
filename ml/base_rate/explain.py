    ...
    try:
        ...
        try:
            with connection.cursor() as cursor:
                cursor.execute("DELETE FROM economic_indicator_contribution WHERE type = 'base_rate'")
                sql = """
                INSERT INTO economic_indicator_contribution (type, variable, weight)
                VALUES (%s, %s, %s)
                """
                for var, weight in contributions.items():
                    cursor.execute(sql, ("base_rate", var, float(weight)))
            connection.commit()
            print(f"[DB] Successfully saved base_rate SHAP contributions to MySQL ({len(contributions)} features).")
        finally:
            connection.close()
