...
* **2단계: AI Target Selector 정교한 대상 선별 (`batch_target_selector_system.md`)**
  ...
  - **선별 우선순위 기준**:
    ...
    4. **정기 점검 및 주기 만료 대상의 사후 관리 (우선순위 4)**: 오랫동안 케어(30일 경과)가 없었던 고객에 대해 분석 연속성을 유지하도록 고려.
  - **최소 선정 비율 보장 지침**: 전체 배치가 중단 없이 활성화되도록 1차 후보군 중 **최소 5명 ~ 최대 15명 범위** 내의 고객은 반드시 최종 분석 대상으로 선정하도록 보장. 만약 후보군이 5명 미만인 소수인 경우에도 **최소 1명 ~ 최대 5명(후보군 전체) 범위** 내에서 반드시 최종 분석 대상으로 선정. 최우선 순위 대상자만으로 최소 선정 비율이 충족되지 않을 경우 우선순위 4 대상자 중 자산 규모가 큰 우량 고객 순으로 채워 충족시킵니다.
